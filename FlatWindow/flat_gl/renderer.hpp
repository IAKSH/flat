...
namespace dggen
{
    ...
    struct Font {
        virtual ~Font() {}
        virtual Font& operator()(Font& font) = 0;
    };
    ...
    template <typename T> struct Renderer {
        template <DrawArgs U> Renderer& operator<<(U&& u)
        {
            ...
            if constexpr(std::same_as<UType, Color>)
                static_cast<T*>(this)->imp_setColor(u);
            else if constexpr(std::same_as<UType,Texture>)
                static_cast<T*>(this)->imp_bindTexture(u);
            else if constexpr(std::same_as<UType, Rectangle>)
                static_cast<T*>(this)->imp_drawRectangle(u);
            else if constexpr(std::same_as<UType, Font>)
                static_cast<T*>(this)->imp_drawText(u);
            else if constexpr(std::same_as<UType, decltype(flush)>)
