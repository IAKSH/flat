...
namespace flat::imp::audio::alsoft
{
	...
	class AudioSource : public flat::AudioSource
	{
	private:
		uint32_t sourceId;
		int offset;
	...
	public:
		...
		virtual std::function<void(float)> operator[](flat::AudioAttribType type) override
		{
	...
	class AudioMixer : public flat::AudioMixer<AudioMixer>
	{
	private:
		ALCdevice* device;
		ALCcontext* context;
	...
	public:
		...
		void imp_playAudioFromSource(flat::AudioSource& _source)
		{
		...
		void imp_setAttrib(flat::AudioAttrib& attrib)
		{
			float oriPos[3];
			float oriVel[3];
		...
		std::unique_ptr<flat::AudioSource> imp_genAudioSource()
		{
			uint32_t sourceId;
		...
		void imp_stopAudioSource(flat::StopFlag& flag)
		{
		...
		void imp_resumeAudioSource(flat::ResumeFlag& flag)
		{
			if (typeid(flag.getAudioSource()) != typeid(AudioSource))
			{
